                                                    ...
                                                    @if ($addon->is_based_on_quantity)
                                                        <div class="quantity-control">
                                                            <label for="addon_quantity-{{ $addon->id }}"
                                                                ...
                                                                <i class="fa fa-sort-numeric-up me-1"></i>How many
                                                                {{ $addon->name }}
                                                                @if ($addon->capacity)
                                                                    <small class="text-muted">(Max:
                                                                        {{ $addon->capacity }})</small>
                                                                @endif
                                                            </label>
                                                ...
                                                <button class="accordion-button collapsed" type="button"
                                                    ...
                                                    <div
                                                        ...
                                                        <div>
                                                            <strong>{{ $addon->name }}</strong>
                                                            @if ($addon->capacity)
                                                                <small class="text-muted">(Max: {{ $addon->capacity }}
                                                                    per person/unit)</small>
                                                            @endif
                                                        </div>
                                                        <div>
                                                    ...
                                                    @if ($addon->is_based_on_quantity)
                                                        <div class="quantity-control">
                                                            <label for="addon_quantity-{{ $addon->id }}"
                                                                ...
                                                                @if ($addon->quantity)
                                                                    <small class="text-muted">(Max:
                                                                        {{ $addon->quantity }} available)</small>
                                                                @endif
                                                            </label>
                                                            <input id="addon_quantity-{{ $addon->id }}"
                                                                type="number"
    ...
    <script>
        ...
        function updatePerItemInputs(addonId, hasSelectedDates) {
            ...
            const checkboxInput = document.querySelector(
                `input[name="addon_checkbox[${addonId}]"][data-billing-cycle="per_day"]`);
            if (quantityInput) {
        ...
        function lockPerContractControlsByCapacity(addonId) {
            ...
            if (checkbox) {
                ...
                checkbox.title = outOfCapacity ? 'No remaining capacity' : '';
            }
            if (qtyInput) {
                qtyInput.disabled = outOfCapacity || qtyInput.disabled;
                if (outOfCapacity) qtyInput.value = 0;
                qtyInput.title = outOfCapacity ? 'No remaining capacity' : '';
            }
        }
        ...
        function initializeAddonsScript() {
            ...
            function validateCheckboxRequired(addonId) {
                ...
                if (addon && addon.price_type === 'flat_rate' && addon.billing_cycle === 'per_contract') {
                    return true;
                }
                        ...
                        if (addon) {
                            ...
                            if (addon.price_type === 'per_night') {
                                totalPrice = calculatePerNightPrice(addonId, addon.base_price, 0, days);
                                    ...
                                    .price_type === 'flat_rate' || addon.price_type === 'per_item')) {
