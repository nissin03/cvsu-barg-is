@php
    ...
    if (isset($chartData) && $chartData !== null) {
        $totalSales =
            $chartData['reserved_sales_total'] +
            $chartData['pickedup_sales_total'] +
            $chartData['canceled_sales_total'];
    } else {
        $totalSales = 0;
        $chartData = [
                                ...
                                <div class="col-md-4">
                                    <div class="d-flex gap-2">
                                        ...
                                        <button type="button" class="btn btn-outline-secondary btn-lg"
                                            onclick="resetDateFilter()" style="min-width: 120px;">
                                            <i class="bi bi-x-circle me-1"></i>Clear
                                        </button>
                                        <button type="button" class="btn  btn btn-dark fs-5 py-3 px-4 w-auto"
                                            id="pdf-export-btn" style="min-width: 120px;">
                ...
                <div class="col-md-6">
                    <div class="d-flex gap-2 justify-content-md-end mt-3 mt-md-0 flex-wrap">
                        <form action="{{ route('admin.reports') }}" method="GET" id="filter-form"
                            ...
                            @csrf
                            <select name="month" class="form-select fs-5 py-3 px-4 border-light-subtle rounded"
                                style="width: 140px;">
                            ...
                            <select name="week" class="form-select fs-5 py-3 px-4 border-light-subtle rounded"
                                style="width: 140px;">
                            ...
                            <select name="year" class="form-select fs-5 py-3 px-4 border-light-subtle rounded"
                                style="width: 140px;">
                            ...
                            <div class="row align-items-center">
                                ...
                                <div class="col-md-6 text-end">
                                    <form action="{{ route('admin.download-input-sales') }}" method="POST" id="download-form"
                                        ...
                                        <input type="hidden" name="reserved_sales"
                                            value="{{ $chartData['reserved_sales_total'] }}">
                                        <input type="hidden" name="pickedup_sales"
                                            value="{{ $chartData['pickedup_sales_total'] }}">
                                        <input type="hidden" name="canceled_sales"
                                            value="{{ $chartData['canceled_sales_total'] }}">
                                    </form>
                                </div>
                            </div>
                        </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                var options = {
                    ...
                    series: [{
                            name: 'Total Sales',
                            data: @json($chartData['total_sales'])
                        },
                        {
                            name: 'Reserved Sales',
                            data: @json($chartData['reserved_sales'])
                        },
                        {
                            name: 'Picked Up Sales',
                            data: @json($chartData['pickedup_sales'])
                        },
                        {
                            name: 'Canceled Sales',
                            data: @json($chartData['canceled_sales'])
                        }
                    ],
                    xaxis: {
